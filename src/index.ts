    ...
    tools: [
      {
        ...
        description: "システム内のCPU監視ログファイル（JSON形式）を検索する",
        inputSchema: {
          type: "object",
          properties: {
            searchPath: {
            ...
            namePattern: {
              type: "string",
              description:
                "ファイル名パターン（省略時は 'process', 'cpu', 'log' を含むファイルを検索）",
            },
          },
        },
      },
      {
        ...
        description: "CPU監視ログファイル（JSON形式）を読み込み、データを取得する",
        inputSchema: {
          type: "object",
          properties: {
            filePath: {
...
// Markdownレポート生成関数
export function generateMarkdownReport(processLogData: any, analysis: any, options: any): string {
...
`;
    analysis.highLoadProcesses.slice(0, 10).forEach((proc: any, index: number) => {
...
async function findProcessLogFiles(searchPath?: string, namePattern?: string): Promise<string[]> {
  ...
  const patterns = namePattern
    ?.toLowerCase()
    .split(",")
    .map((p) => p.trim()) || ["process", "cpu", "log", "monitor"];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isFile() && extname(entry.name).toLowerCase() === ".json") {
          ...
          const hasMatchingPattern = patterns.some((pattern) => fileName.includes(pattern));
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
        ...
        content: [
          {
            type: "text",
            text:
              `🔍 CPU監視ログファイルを${foundFiles.length}個発見しました\n\n` +
    ...
    if (name === "loadProcessLogFile") {
      ...
      // データ構造の基本検証
      if (!processLogData.measurements || !Array.isArray(processLogData.measurements)) {
        throw new Error("無効なデータ形式です。measurements配列が見つかりません。");
      }
    ...
    if (name === "analyzeProcessData") {
      ...
      // データ検証
      if (!processLogData.measurements || !Array.isArray(processLogData.measurements)) {
        throw new Error("processLogData.measurementsが存在しないか、配列ではありません");
      }
